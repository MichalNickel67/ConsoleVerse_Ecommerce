...
<body>
    ...
    <div class="container mt-3">
        <div class="row justify-content-between align-items-center">
             <h1 class="mb-0"><b>List of ConsoleVerse Products </b></h1>
        </div>
    </div>

    <div class='container mt-3'>
        <div class='text-center'>
            <a href='AddProductFORM.php' class='btn btn-outline-success mx-auto'>Add New Product</a>
        </div>
    </div>
    ...
    <?php
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<img src='".$row["img"]."' class='card-img-top' alt='".$row["brand"]." ".$row["model"]."'>";
                echo "<div class='card-body'>";
                echo "<h5 class='card-title'>".$row["brand"]." ".$row["model"]."</h5>";
                echo "<p class='card-text'>ID: ".$row["prod_id"]."<br>"."Price: £".$row["price"]."<br>".$row["short_desc"]."</p>";
                echo "<p style='font-weight: bold; color: green;'>Available: ".$row['stock']."</p>";
                echo "<div class='card-buttons d-flex justify-content-end'>";
                echo "<a href='ModifyProductFORM.php?prod_id=".$row["prod_id"]."' class='btn btn-warning mx-2'>Edit Product</a>";
                echo "<a href='DeleteProduct_Script.php?prod_id=".$row["prod_id"]."' class='btn btn-danger'>Delete Product</a>";
                echo "</div>";
                echo "</div>";
                echo "</div>";
                echo "</div>";
