        ...
        <div class="col-md-8">
            <div class="card shadow-lg">
                <div class="card-body text-center">
                    <h1 class="mb-3"><b>Order Placed Successfully</b></h1>
                    <?php
                        $sql = "SELECT order_number FROM orderhistory ORDER BY order_id DESC LIMIT 1";
                        $result = mysqli_query($con, $sql);

                        if($result) {
                            if(mysqli_num_rows($result) > 0) {
                                $row = mysqli_fetch_assoc($result);
                                echo "<p class='lead'>Your Order Number is: <b>" . $row["order_number"] . "</b></p>";
                                echo "<a href='ViewOrders.php' class='btn btn-primary mt-3'>View Orders</a>";
                            }
                            else {
                                echo "<p class='lead'>No order has been placed yet.</p>";
                            }
                        }
                        else {
